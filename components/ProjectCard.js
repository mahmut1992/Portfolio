    ...
    >
      <div className="w-full h-full p-4 bg-white rounded-lg dark:bg-gray-900">
        <div className="flex flex-col items-center space-y-4 ">
          ...
          <p className="text-md min-h-[168px] ">{description}</p>

          <div className="flex items-center gap-2 flex-wrap">
            {tags.map((tag, key) => (
              <span
                key={key}
                className="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-pink-100 bg-pink-700 rounded h-10 text-center"
              >
                {tag}
              </span>
            ))}
